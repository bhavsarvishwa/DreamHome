<?php
    ...
    $description = "This is dashboard file for logged in agents.";
    $banner = "Dashboard";
...
// $dashboard_array = "";
// $dashboard_array = array($username, OPEN, CLOSED);
// dump($dashboard_array);
// $dashboard = pg_execute($conn, 'dashboard', $dashboard_array);

// $listing_results = pg_fetch_all($dashboard);

// dump($dashboard);


 $listing_select = pg_query($conn,"SELECT * FROM listings WHERE user_id = '.$username.' AND listing_status = 'o' OR (user_id = 'savaliyad' AND listing_status = 'c') LIMIT ".RECORDS_RETURNED);
 //dump($listing_select);
